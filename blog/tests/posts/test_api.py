...
class TestCreatePost:
    def test_anonymous_user_can_not_create_book_return_401(self, api_client, media_root):
        response = api_client.post(post_url)

        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_authenticated_user_can_create_post_return_201(self, api_client, post_factory, user_factory, media_root):
    ...
    def test_data_is_invalid_return_400(self, api_client, post_factory, user_factory, media_root):
        author = user_factory.create()
        post1 = post_factory.create(author=author)
        api_client.force_authenticate(author)
        post_data = {
...
class TestGetPost:
    ...
    def test_if_post_exist_return_200(self, api_client, create_post, media_root):
        ...
        assert response_data['published_at'] == create_post.published_at.strftime('%Y-%m-%d %H:%M:%S')
    ...
    def test_filter_posts_return_200(self, api_client, post_factory, category_factory, media_root):
...
class TestUpdatePost:
    def test_anonymous_user_can_not_update_post_return_401(self, api_client, create_post, media_root):
        response = api_client.put(f'{post_url}{create_post.slug}/')

        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_authenticated_user_can_not_update_others_posts_return_403(self, api_client, user_factory, post_factory, media_root):
    ...
    def test_update_posts_return_200(self, api_client, user_factory, post_factory, media_root):
        user = user_factory.create()
        post = post_factory.create(author=user)
        updated_post = post_factory.create(author=user)
        updated_post.slug = 'new_slug'
        api_client.force_authenticate(user)
        response = api_client.patch(f'{post_url}{post.slug}/', model_to_dict(updated_post))
...
class TestDeletePost:
    def test_anonymous_user_can_not_delete_post_return_401(self, api_client, create_post, media_root):
        response = api_client.delete(f'{post_url}{create_post.slug}/')

        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_authenticated_user_can_not_delete_others_posts_return_403(self, api_client, user_factory, post_factory, media_root):
        user1 = user_factory.create()
        user2 = user_factory.create()
        post = post_factory.create(author=user1)
        api_client.force_authenticate(user2)
    ...
    def test_delete_post_return_204(self, api_client, user_factory, post_factory, media_root):
        user = user_factory.create()
        post = post_factory.create(author=user)
        api_client.force_authenticate(user)
        response = api_client.delete(f'{post_url}{post.slug}/')
