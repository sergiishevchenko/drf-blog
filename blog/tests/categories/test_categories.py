...
class TestCreateCategory:
    ...
    def test_common_user_can_not_create_category_return_403(self, api_client, user_factory):
...
class TestUpdateCategory:
    def test_anonymous_user_can_not_update_category_return_401(self, api_client):
        response = api_client.put(category_url)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_common_user_can_not_update_category_return_403(self, api_client, user_factory, create_category):
        user = user_factory.create()
        api_client.force_authenticate(user=user)
        response = api_client.put(f'{category_url}{create_category.id}/', data={'name': 'update category'})
        assert response.status_code == status.HTTP_403_FORBIDDEN

    def test_admin_user_update_category_return_201(self, api_client, category_factory):
        category = category_factory.create()
        api_client.force_authenticate(user=User(is_staff=True))
        response = api_client.put(f'{category_url}{category.id}/', data={'name': 'update category'})
...
class TestDeleteCategory:
    def test_anonymous_user_can_not_delete_category_return_401(self, api_client):
        response = api_client.delete(category_url)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_common_user_can_not_delete_category_return_403(self, api_client, user_factory, create_category):
        user = user_factory.create()
        api_client.force_authenticate(user=user)
