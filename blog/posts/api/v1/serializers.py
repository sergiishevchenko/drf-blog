...
from posts.models import Category, Comment, Post, FavoritePost
...
class PostSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Post
        fields = ['id', 'author', 'category', 'title', 'slug', 'content', 'status', 'image',
                  'likes', 'comments', 'absolute_url', 'created_at', 'updated_at', 'published_at',]
...
class SimplePostSerializer(serializers.ModelSerializer):
    ...
    def get_absolute_url(self, obj):
        request = self.context.get('request')
        if obj:
            return request.build_absolute_uri(reverse('posts:api-v1:posts-detail', args=[obj.slug]))

    class Meta:
        model = Post
        fields = ['id', 'author', 'category', 'title', 'likes', 'absolute_url',]
