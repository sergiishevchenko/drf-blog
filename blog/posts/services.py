...
def create_post(user, category, title, content, image, status, published_at):
    return Post.objects.create(author=user, category=category, title=title, slug=slugify(title),
                               content=content, image=image, status=status, published_at=published_at)
